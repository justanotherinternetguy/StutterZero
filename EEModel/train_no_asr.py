...
def main():
    ...
    criterion = LossNoASR(spec_criterion)
    ...
    for epoch in range(config.training.begin_epoch, config.training.end_epoch):
        ...
        with open("./ee_no_asr.txt", "a") as f:
            f.write("lr = " + str(lr))
            f.write('\n')
        ...
        if pre_test_loss > eval_loss:
            torch.save(model.module.state_dict(), "./plz_load/best_ee_no_asr.pth")
            pre_test_loss = eval_loss
        
        torch.save(model.module.state_dict(), "./plz_load/ee_no_asr.pth")               
